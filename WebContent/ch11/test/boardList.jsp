...
<%
	...
	ArrayList<BoardVO> listOfBoard = boardRepository.selectBoardList();
	request.setAttribute("listOfBoard", listOfBoard);

%>
<h1>게시판 목록</h1>
<form action="<%=request.getContextPath()%>/ch11/test/boardForm.jsp">
<table border='1'>
	<tr>
		<th>제목</th>
		<th>작성자</th>
...
<%		
	...
	if(listOfBoard.size() == 0) {
		
%>

	<tr><td colspan="4"> 조회하실 게시글이 존재하지 않습니다.</td></tr>
...
%>
	...
	<c:forEach items="${listOfBoard}" var="board">
		<tr>
		   	<td><a href="${pageContext.request.contextPath}/ch11/test/boardView.jsp?no=${board.no}">${board.title}</a></td>
		    <td>${board.writer}</td>
		 	<td>${board.regDate}</td>
		    <td>${board.hit}</td>  
